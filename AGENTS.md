...
- QSMxT 9.22.0-ais.2 exposes OpenRecon pipeline presets and custom algorithm dropdowns using Container Service `select-one`/`select-values`. The default is Custom with HD-QSM, ROMEO, iSMV, and BET/magnitude masking. SWI/T2*/R2* generation defaults on; ARLO fitting requires at least three equally spaced magnitude echoes and computes both fitted maps together. `part-mag_T2starw` remains combined magnitude contrast, not a relaxation map; NIfTI T2* is in seconds and R2* in 1/s. Preserve the `premade` input name for existing configurations; legacy choices are retained. When updating an existing command, also inspect site/project wrapper configuration overrides (`default-value`, `user-settable`) and verify the launch UI exposes the new controls. The wrapper backfills source SeriesNumber into all derivative sidecars. The scan-link sync recognizes compressed/uncompressed files with/without BIDS sessions and links Chimap/SWI+minIP/T2starmap/R2starmap to QSM/SWI/T2STAR/R2STAR scan resources respectively. QSMxT 9.22.0 features exposed as launch inputs: Generate SMWI (default on; writes `desc-paramagnetic`/`desc-diamagnetic` `_smwi` plus their own `_minIP`, with a smaller projection window than SWI), Generate R2' map with R2' source (`auto`/`mese`/`r2primenet`; R2PRIMEnet output is a prediction), Two-pass artefact reduction (keeps `desc-singlepass_Chimap`), SynthSeg segmentation (`dseg.nii` + `dseg.tsv`), per-structure statistics (`desc-segmentation_qsmstats.tsv`, implies segmentation), the LSQR/HEIDI inversions, and the `bet-and-phase`/`hd-bet` mask presets. `--masking-input` is passed only with the `bet` and `robust-threshold` presets: it overrides the input of *every* mask section, so passing it with `bet-and-phase` would collapse the intersection onto one input. The SynthSeg and R2PRIMEnet weights ship in the image (verified with `--network none`). QSMxT 9.22.0's `--clean-intermediates` deletes the final `R2primemap` along with the intermediates, so the wrapper no longer passes it and instead deletes everything under `derivatives/qsmxt/workflow/` except `provenance.json` before upload; re-check this when bumping. The scan-link sync maps by `desc-<label>_<suffix>` first, then by suffix: Chimap and two-pass `desc-singlepass_Chimap` -> `QSM`, `swi`/`minIP` -> `SWI`, SMWI images and minIPs -> `SMWI`, `T2starmap` -> `T2STAR`, `R2starmap` -> `R2STAR`, `R2primemap` -> `R2PRIME`, `dseg` (with its `.tsv` label table) -> `SEG`; `desc-reliable_mask`, `mask`, `T2starw` and the stats TSV stay session-level only. Test with `python3 -m unittest discover -s tests -p 'test_qsmxt*.py'` and an isolated installed-image synthetic multi-echo run; a small spherical phantom needs threshold masking rather than human-brain BET.
...
Offload is age-gated by `OFFLOAD_MIN_AGE_HOURS` (default and CronJob value `24`, added 2026-09-23): a session whose newest local regular file is younger than that is still rsynced to GCS every night but logged `DEFER`, keeps its local files, and gets **no** `.backup_complete` marker, so the next run re-syncs it incrementally and offloads it once old enough. With the 02:00 schedule, data therefore stays on the XNAT disk for 24–48h after its last write. Appending files resets the clock, since the gate uses the same newest-mtime check as `STALE`. Because the marker now lands a night or two later, the ingest wrapper's "marker newer than staged → delete staged outright" shortcut fires less often and more sessions pass through `uploaded/` (still pruned after 7 days by `ingest-uploaded-retention`). The run summary reports the deferred count; deferrals are not failures.
...
### QSMxT minIP header bug (QSMxT#211)

QSMxT 9.19.1–9.21.x compute the SWI minIP as a seven-slice sliding minimum with depth `nz-6` but save it through the full-volume writer with depth `nz`, so the NIfTI header promises more voxels than the payload holds and viewers show nothing. Command version 9.19.1-ais.3 (2026-09-18) worked around it by regenerating the minIP from the saved SWI. The fix shipped upstream in QSMxT v9.22.0 (QSMxT PR #212, 2026-09-22): the minIP is written on its own shorter grid with each slice at its slab centre, in both `qsmxt run` and `qsmxt swi`. Command version 9.22.0-ais.1 therefore drops the regeneration and keeps only a verify-only guard that reads each minIP's complete payload, checks it has `nz-6` slices, and deletes (with a container-log warning) any malformed minIP before upload so a regression can never reach XNAT again. Header-only load/existence checks are insufficient for this class of bug. `tests/qsmxt_phantom.py` asserts QSMxT's own minIP shape, values, and slab-centre origin with no repair step; it passes on the 9.22.0 binary and fails on 9.19.1. Neurodesk published `vnmd/qsmxt_9.22.0:20260923` the same day; command 9.22.0-ais.2 was installed on it on 2026-09-23 (as command id 9 only, see below). A new upstream binary can be pre-tested before its Neurodesk image exists by mounting the GitHub release binary over the current image's `/opt/qsmxt/qsmxt` (`-v <binary>:/opt/qsmxt/qsmxt:ro`) when running the phantom. Also check the 9.20+ behaviour changes on the smoke-test session: derivatives are resampled back to the acquired grid by default (`--output-space acquired`), oblique acquisitions rotate the dipole kernel instead of assuming B0 along +z, and SWI shifts slightly because Laplacian unwrapping no longer includes a background-removal pass. The only malformed minIPs in the archive were the two QSMxT 9.19.1 outputs of `openrecon/260916_qsm_spine_topo_synthseg_51993533_7eb7_4e` (`XNAT_E00440`, OpenRecon GRE scans 12 and 21, plus their scan-level `SWI` links). On 2026-09-23 the session's complete `QSMXT` resource and the derived `QSM`/`SWI`/`T2STAR`/`R2STAR` link resources on scans 12 and 21 were deleted through REST (`removeFiles=true`) and QSMxT 9.22.0-ais.2 was rerun (container 58) with the previous run's settings (`premade=romeo-resharp-rts`, BET/magnitude, SWI/T2*/R2*) plus SMWI. The whole resource was replaced, not just the minIPs: every file had already been offloaded to a symlink into the read-only object store, so an in-place re-upload over the same names would write through those symlinks, and the scan-link sync skips already-registered files, so the old scan links would never have pointed at the new outputs. The pre-rerun bytes remain in GCS under `sessions/openrecon/260916_qsm_spine_topo_synthseg_51993533_7eb7_4e/RESOURCES/QSMXT/` (58 objects); the archiver's rsync never deletes GCS objects. After the rerun all six minIPs pass the full-payload check and the link sync attached `QSM`, `SWI`, `SMWI`, `T2STAR`, and `R2STAR` to both scans with no errors. Use the same delete-then-rerun approach for any future reprocess of an offloaded session.

On 2026-09-23 `scripts/install-mriqc-container-service.sh` failed on its first upsert with HTTP 500 (`ConstraintViolationException`, unique constraint `uk4eehacikolu0x73oqruq4bwij`). The cause was that the stray duplicate commands `dcm2niix` v1.5 (id 17, wrapper 16 `dcm2niix-scan`) and `xnat2bids` v1.3 (id 16) had reappeared: the installer resolves commands by name, picked id 17, and the update collided with command 6's wrapper names. No container had ever run on either, so both were deleted (`DELETE /xapi/commands/<id>`) the same day, after which the full installer completed cleanly with 10 commands and no duplicate names. If the installer hits this constraint again, check `GET /xapi/commands` for duplicate names first.
