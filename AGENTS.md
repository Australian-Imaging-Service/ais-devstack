...
If XNAT shows many failed `Pulled Data from DICOM` workflows after ais-edge uploads, check the `xnat-upload/xnat-ingest-upload` deployment. Catalog-only `DICOM` resources in the archive make XNAT's `pullDataFromHeaders=true` endpoint fail with `Unable to locate DICOM or ECAT files`; the ingest uploader should skip that header-pull step unless local DICOM objects are actually present. Another failure mode is `Modification of scan modality (xnat:mrScanData to xnat:otherDicomScanData) not supported`: a DICOM Presentation State (`Modality=PR`, such as Siemens `*_PR` series) was created as an MR scan before header extraction. The live uploader patch `ais-devstack/pr-scan-modality=xnat-ingest-pr-other-dicom-v1` maps `PR` to XNATPy's `OtherDicomScanData` at scan creation. To repair an existing session, first verify the retained `uploaded/` object and its DICOM header, delete only the affected PR scan with `removeFiles=true`, recreate it as `xnat:otherDicomScanData`, re-upload the retained DICOM, and rerun `pullDataFromHeaders=true`. XNAT then moves the PR object from `DICOM/` into the scan's `secondary/` directory; that is the expected final location. A failed earlier pull can leave a pre-existing secondary copy that collides with the repaired DICOM (`FileAlreadyExistsException`): verify that old copy byte-for-byte against the retained source, remove only that stale pre-repair `secondary/` directory, and rerun header extraction so XNAT creates a fresh secondary copy. Require a `Complete` workflow, the correct `xnat:otherDicomScanData` type and PR metadata, and a source-matching file in the final `secondary/` directory. Active stale failures can be dismissed by marking `wrk_workflowdata.status` as `Failed (Dismissed)` for `pipeline_name='Pulled Data from DICOM'`.
