...
### Prepared QSMxT minIP header workaround (2026-09-18)

QSMxT 9.19.1 computes a seven-slice sliding minimum with depth `nz-6`, but saves it through the full-volume writer with depth `nz`. The resulting NIfTI has a truncated payload relative to its header and can appear blank. Prepared command version 9.19.1-ais.3 regenerates minIP from saved SWI with the correct shape and moves the affine origin to the first slab centre. It reads the complete output back before upload. Keep this workaround until an image bump passes `tests/test_qsmxt_minip.py`; header-only load/existence checks are insufficient. Deployment and repair of the two existing minIPs await explicit user approval after automatic approval review rejected production mutations. Their voxel payloads exactly match regenerated SWI projections; only the first 352 header bytes require repair. Preserve full backups outside the archive and preserve existing scan hard links if repair is approved.
