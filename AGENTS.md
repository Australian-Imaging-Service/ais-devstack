...
`scripts/install-mriqc-container-service.sh` currently fails on its first upsert with HTTP 500 (`ConstraintViolationException`, unique constraint `uk4eehacikolu0x73oqruq4bwij`) because the stray duplicate commands `dcm2niix` v1.5 (id 17, wrapper 16 `dcm2niix-scan`) and `xnat2bids` v1.3 (id 16) have reappeared. The installer resolves commands by name, picks id 17, and the update collides with command 6's wrapper names. Delete the non-repo duplicates as described in the Container Service bullet above before running the installer again. On 2026-09-23 only QSMxT was upserted directly (`POST /xapi/commands/9` with the repo JSON plus `"id": 9`), so the duplicates are still present.
