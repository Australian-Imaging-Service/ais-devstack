...
QSMxT 9.19.1–9.21.x compute the SWI minIP as a seven-slice sliding minimum with depth `nz-6` but save it through the full-volume writer with depth `nz`, so the NIfTI header promises more voxels than the payload holds and viewers show nothing. Command version 9.19.1-ais.3 (2026-09-18) worked around it by regenerating the minIP from the saved SWI. The fix shipped upstream in QSMxT v9.22.0 (QSMxT PR #212, 2026-09-22): the minIP is written on its own shorter grid with each slice at its slab centre, in both `qsmxt run` and `qsmxt swi`. Command version 9.22.0-ais.1 therefore drops the regeneration and keeps only a verify-only guard that reads each minIP's complete payload, checks it has `nz-6` slices, and deletes (with a container-log warning) any malformed minIP before upload so a regression can never reach XNAT again. Header-only load/existence checks are insufficient for this class of bug. `tests/qsmxt_phantom.py` asserts QSMxT's own minIP shape, values, and slab-centre origin with no repair step; it passes on the 9.22.0 binary and fails on 9.19.1. **The `vnmd/qsmxt_9.22.0` Neurodesk image did not exist yet on 2026-09-23** (the neurocontainers auto-update had not opened a bump PR), so the command's image tag is the placeholder `PENDING` — replace it (`grep -rn qsmxt_9.22.0:PENDING`) with the published tag before installing the command. Until then a binary can be pre-tested by mounting the upstream release over the current image's `/opt/qsmxt/qsmxt` (`-v <qsmxt-v9.22.0 binary>:/opt/qsmxt/qsmxt:ro`) when running the phantom. Also check the 9.20+ behaviour changes on the smoke-test session: derivatives are resampled back to the acquired grid by default (`--output-space acquired`), oblique acquisitions rotate the dipole kernel instead of assuming B0 along +z, and SWI shifts slightly because Laplacian unwrapping no longer includes a background-removal pass. The two minIPs already in XNAT from 9.19.1 runs still carry the malformed header: their voxel payloads exactly match regenerated SWI projections and only the first 352 header bytes need repair, which awaits explicit user approval. Preserve full backups outside the archive and preserve existing scan hard links if repair is approved; alternatively rerun those sessions on 9.22.0.
